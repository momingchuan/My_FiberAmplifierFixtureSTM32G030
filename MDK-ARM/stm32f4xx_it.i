...
extern unsigned char UART1_RX_BUF[1000];
extern unsigned char UART3_RX_BUF[50];
extern unsigned char UART4_RX_BUF[100];
extern unsigned char UART6_RX_BUF[100];
extern unsigned char string[30];

 uint16_t UART_RX_STA;
 unsigned char ClearData[50]={0};
...
if(((CoreDebug_Type *) (0xE000EDF0UL))->DHCSR&1)
{
__asm__ __volatile__ ("bkpt %0" : : "i"(0) );
}
...
void TIM4_IRQHandler(void)
{
 ...
 if(SYS_tickTime.sys_ReceiveData_ms==120-2)
 {
  ...
  USART_Data.USART4_Data.US4_ReceiDataSucce_Flag = 1;
  UART_RX_STA = 100 - ((huart4.hdmarx)->Instance->NDTR);


  memcpy(USART_Data.USART4_Data.US4_receiveData,UART4_RX_BUF,100);
# 387 "../Core/Src/stm32f4xx_it.c"
  memset(UART4_RX_BUF,0,sizeof(UART4_RX_BUF));
  HAL_UART_Receive_DMA(&huart4, UART4_RX_BUF, 100);
 }
...
static char letter='/';
 ...
 if((((&huart1)->Instance->SR & (((uint32_t)(0x1UL << (4U))))) == (((uint32_t)(0x1UL << (4U))))) != RESET)
 {
  USART_Data.USART1_Data.US1_ReceiDataSucce_Flag = 1;
     do{ volatile uint32_t tmpreg = 0x00U; tmpreg = (&huart1)->Instance->SR; tmpreg = (&huart1)->Instance->DR; (void)tmpreg; } while(0U);
     HAL_UART_DMAStop(&huart1);
     UART_RX_STA = 1000 - ((huart1.hdmarx)->Instance->NDTR);




  if(!(UART1_RX_BUF[0]==0x1a&& UART1_RX_BUF[1]==0xff&&UART1_RX_BUF[2]==0xff))
  {
   memcpy(USART_Data.USART1_Data.US1_ReceiveDataStr,UART1_RX_BUF,1000);
   ...
   USART_Data.USART1_Data.US1_CutApartBuf,&letter);
     ...
     HAL_UART_Receive_DMA(&huart1, UART1_RX_BUF, 1000);
 }
...
void USART3_IRQHandler(void)
{
 ...
 if((((&huart3)->Instance->SR & (((uint32_t)(0x1UL << (4U))))) == (((uint32_t)(0x1UL << (4U))))) != RESET)
 {
  USART_Data.USART3_Data.US3_ReceiDataSucce_Flag = 1;
  ...
  = strtod((char *)USART_Data.USART3_Data.US3_ReceiveDataStr[USART_Data.USART3_Data.US3_ReceiDataSucceCount],0);

memset(UART3_RX_BUF,0,sizeof(UART3_RX_BUF));

  HAL_UART_Receive_IT(&huart3,UART3_RX_BUF,50);
...
void UART4_IRQHandler(void)
{
 ...
 if((((&huart4)->Instance->SR & (((uint32_t)(0x1UL << (4U))))) == (((uint32_t)(0x1UL << (4U))))) != RESET)
 {
  do{ volatile uint32_t tmpreg = 0x00U; tmpreg = (&huart4)->Instance->SR; tmpreg = (&huart4)->Instance->DR; (void)tmpreg; } while(0U);
...
void USART6_IRQHandler(void)
{
 ...
 if((((&huart6)->Instance->SR & (((uint32_t)(0x1UL << (4U))))) == (((uint32_t)(0x1UL << (4U))))) != RESET)
 {
  USART_Data.USART6_Data.US6_ReceiDataSucce_Flag = 1;
     do{ volatile uint32_t tmpreg = 0x00U; tmpreg = (&huart6)->Instance->SR; tmpreg = (&huart6)->Instance->DR; (void)tmpreg; } while(0U);
  ...
  = strtod((char *)USART_Data.USART6_Data.US6_ReceiveDataStr[USART_Data.USART6_Data.US6_ReceiDataSucceCount],0);
  ...
  HAL_UART_Receive_IT(&huart6,UART6_RX_BUF,100);
...
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart)
{
 ...
 }else if(huart->Instance == ((USART_TypeDef *) ((0x40000000UL + 0x00010000UL) + 0x1400UL)))
 {
   USART_Data.USART6_Data.US6_SendDataSucce_Flag=1;
 }else if(huart->Instance == ((USART_TypeDef *) (0x40000000UL + 0x4C00UL)))
 {
